...
const Calendar = () => {
  ...
  const handleEventClick = async (selected: EventClickArg) => {
    if (window.confirm(`Are you sure you want to delete the appointment "${selected.event.title}"?`)) {
      const deletedEvent = {
      ...
      try {
        await fetch("http://localhost:7000/api/email/sendemail", {
          ...
          body: JSON.stringify({
            to: "mootezaouinti1@gmail.com",
            subject: `Appointment Canceled: ${deletedEvent.title}`,
            message: `The appointment "${deletedEvent.title}" scheduled on ${formatDate(deletedEvent.start, {
              year: "numeric",
              month: "long",
              day: "numeric",
            })} has been canceled.`,
